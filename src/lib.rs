...
struct GeometricalLevelGenerator {
    total: usize,
    p: f64,
...
/// Implementation of each skiplist node, containing the value, the immediately previous and next
/// nodes, and also a list of links to nodes further down the list.
///
/// The `next` owns the next node.
#[derive(Clone, Debug)]
struct SkipNode<T> {
    // value should never be None, with the sole exception being the head node.
    value: Option<T>,
    // how high the node reaches.  This should be equal to the vector length.
    level: usize,
    // The immediately next element (and owns that next node).
    next: Option<Box<SkipNode<T>>>,
    // The immediately previous element.
    prev: Option<*mut SkipNode<T>>,
    ...
    links: Vec<Option<*mut SkipNode<T>>>,
...
impl<T> fmt::Display for SkipNode<T>
        where T: fmt::Display {
...
impl<T> SkipList<T> where
    ...
    ///
    /// The comparison function can always be changed with `order_by`, which has essentially no
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        let levels = std::num::Float::floor(std::num::Float::log2(capacity as f64)) as usize;
...
impl<T> SkipList<T> {
    ...
    /// use skiplist::SkipList;
    ///
    /// let mut skiplist = SkipList::with_comp(|a: &f64, b: &f64| {
    ...
    ///
    /// As a result, `order_by` is safe to call if the skiplist is empty or has just a single
    ...
    ///
    /// let mut skiplist = SkipList::new();
    /// skiplist.order_by(|a: &i64, b: &i64| b.cmp(a)); // All good; skiplist empty.
    /// skiplist.insert(0);
    /// skiplist.insert(10);
    /// skiplist.order_by(|a: &i64, b: &i64| a.cmp(b)); // Panics; order would change.
    /// ```
    pub fn order_by<F>(&mut self, f: F) where
            F: 'static + Fn(&T, &T) -> Ordering {
        unsafe {
    ...
    pub fn insert(&mut self, value: T) {
        unsafe {
            ...
            for (lvl, &insert_node) in insert_nodes.iter().rev().enumerate() {
                if lvl == 0 {
                    (*insert_node).links_len[lvl] =
                        if (*insert_node).is_head() {
                            0
                        } else {
                            1
                        };
                    ...
                    let length = self.link_length(insert_node, Some(new_node_ptr), lvl);
    ...
    /// let mut skiplist = SkipList::new();
    /// skiplist.extend(0..10);
    /// assert_eq!(skiplist.remove_value(&4), Some(4)); // Removes the last one
    /// assert_eq!(skiplist.remove_value(&4), None); // No more '4' left
    /// ```
    pub fn remove_value(&mut self, value: &T) -> Option<T> {
        ...
        unsafe {
            ...
            let mut prev_nodes: Vec<*mut SkipNode<T>> = Vec::with_capacity(self.level_generator.total());
    ...
    /// frequently inserted and removed, this method should be preferred over `remove_value`.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///     skiplist.extend(0..10);
    /// }
    /// assert_eq!(skiplist.remove_value(&15), None);
    /// for _ in 0..9 {
    ///     for i in 0..10 {
        ...
        unsafe {
            ...
            if let Some(return_node) = return_node {
                ...
                mem::replace(
                    ...
                    mem::replace(
                        &mut (*return_node).next,
                        None)).expect("Popped node shouldn't be None.").into_inner()
            } else {
                None
            }
        }
...
impl<T> SkipList<T>
    ...
    fn debug_structure(&self) {
        unsafe {
            let mut node: *const SkipNode<T> = mem::transmute_copy(&self.head);
            let mut rows: Vec<_> = iter::repeat(String::new()).take(self.level_generator.total()).collect();
...
impl<T: PartialOrd> std::default::Default for SkipList<T> {
...
impl<T> iter::FromIterator<T> for SkipList<T> where
        T: PartialOrd {
    #[inline]
...
mod tests {
    ...
    #[test]
    fn basic_small() {
        let mut sl: SkipList<i64> = SkipList::new();        sl.check();
        assert_eq!(sl.remove_value(&1), None);              sl.check();
        sl.insert(1);                                       sl.check();
        assert_eq!(sl.remove_value(&1), Some(1));           sl.check();
        sl.insert(1);                                       sl.check();
        sl.insert(2);                                       sl.check();
        assert_eq!(sl.remove_value(&1), Some(1));           sl.check();
        assert_eq!(sl.remove_value(&2), Some(2));           sl.check();
        assert_eq!(sl.remove_value(&1), None);              sl.check();
    }
    ...
    fn basic_large() {
        ...
        for i in 0..size {
            assert_eq!(sl.remove_value(&i), Some(i));
    ...
    fn range_1000() {
        ...
        fn test(sl: &SkipList<u32>, size: u32, min: Bound<&u32>, max: Bound<&u32>) {
